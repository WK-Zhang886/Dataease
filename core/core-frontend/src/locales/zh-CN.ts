export default {
  ...
  online_map: {
    ...
    service_type: '服务类型',
    raster_tile_url: '栅格瓦片 URL',
    vector_style_json: '矢量地图 Style JSON',
    tile_url: '瓦片地址',
    tile_url_placeholder: '例如：http://localhost:18080/{z}/{x}/{y}.png',
    tile_url_tip: '支持标准 {z}/{x}/{y} URL 模板，可通过切片方案兼容 TMS',
    style_url: 'Style JSON 地址',
    style_url_placeholder: '例如：http://localhost:18081/styles/basic-preview/style.json',
    style_url_tip: '请输入完整的 MapLibre Style JSON 地址，内部瓦片、字体和图标资源需允许跨域访问',
    tile_scheme: '切片方案',
    tile_size: '瓦片尺寸',
    min_zoom: '最小层级',
    max_zoom: '最大层级',
    zoom_tip: '限制地图允许操作的缩放范围，不会增加服务本身不存在的地图细节',
    show_attribution: '显示版权说明',
    attribution: '版权说明',
    raster_attribution_tip: '可选，留空时不显示自定义版权说明',
    vector_attribution_tip: '可选，填写后将替换 Style 数据源自带的版权说明；留空则保留原说明',
    map_load_error: '地图加载失败，请检查地址、跨域、资源完整性和网络设置',
    map_network_offline_error: '当前浏览器处于离线状态，请恢复网络后重试',
    map_mixed_content_error:
      '当前页面使用 HTTPS，浏览器已阻止 HTTP 地图资源，请将地图服务切换为 HTTPS',
    map_localhost_error:
      '地图地址使用 localhost，它指向当前访问者的电脑；请确认本机服务已启动，或改为浏览器可访问的服务器地址',
    map_style_request_error: '无法访问 Style JSON，请检查地址、网络连接或跨域设置',
    map_style_http_error: 'Style JSON 请求失败（HTTP {status}），请检查地址和服务状态',
    map_style_json_error: 'Style JSON 内容不是有效的 JSON',
    map_style_format_error: 'Style JSON 格式不完整，必须包含 version 8、sources 和 layers',
    map_tile_http_error: '地图瓦片请求失败（HTTP {status}），请检查地址模板、可用层级和服务状态',
    map_tile_load_error: '地图瓦片加载失败，请检查地址模板、可用层级和跨域设置',
    map_glyph_load_error: '地图字体资源加载失败，请检查 glyphs 地址和跨域设置',
    map_sprite_load_error: '地图图标资源加载失败，请检查 sprite 地址和跨域设置',
    map_resource_http_error: '地图资源请求失败（HTTP {status}），请检查 Style 中的资源地址',
    map_resource_load_error: '地图资源加载失败，请检查 Style 中的资源地址和跨域设置'
  },
  setting_basic: {
